...
// scripts/shiki/build.ts
import { writeFile as writeFile2, mkdir as mkdir2, readFile as readFile3, stat } from "fs/promises";
import { join as join2, dirname as dirname2, relative } from "path";
import { existsSync } from "fs";

// scripts/shiki/processor.ts
...
// scripts/shiki/fileName-transformer.ts
var transformerFileName = ({
  style = "v2",
  hideDot = false
} = {}) => ({
  ...
  pre(node) {
    ...
    node.properties.class = classArray.filter((c) => typeof c === "string" || typeof c === "number");
    node.children.push({
      type: "element",
      tagName: "span",
      properties: {
        class: [
          ...
          hideDot ? "px-2" : "pl-4 pr-2 before:inline-block before:size-1 before:bg-green-500 before:rounded-full before:absolute before:top-[45%] before:left-2",
          style === "v1" ? "left-0 -top-6 rounded-t-md border border-b-0 bg-muted/50" : "left-2 top-(--file-name-offset) border rounded-md bg-background"
        ]
      },
      children: [
        {
...
// scripts/shiki/language-support.ts
import { bundledLanguages } from "shiki";
var LanguageSupport = class {
  constructor() {
    this.supportedLanguages = /* @__PURE__ */ new Map();
   ...
   */
  getAllLanguages() {
    return Array.from(this.supportedLanguages.values()).sort(
  ...
  findSimilar(lang, maxResults = 5) {
    ...
    return results.sort((a, b) => b.score - a.score).slice(0, maxResults).map((r) => r.info);
  }
  /**
   * 获取语言建议消息
   */
  getSuggestionMessage(lang) {
    const similar = this.findSimilar(lang, 3);
    if (similar.length === 0) {
      return `Language "${lang}" is not supported. Use "plaintext" as fallback.`;
    }
    const suggestions = similar.map((info) => {
...
var languageSupport = new LanguageSupport();
async function main() {
...
var isMainModule = process.argv[1] && (import.meta.url.endsWith(process.argv[1]) || import.meta.url.includes(process.argv[1].replace(/\\/g, "/")));
if (isMainModule) {
  main();
}

// scripts/shiki/processor.ts
var ShikiProcessor = class {
  constructor(config, enableLogging = true) {
    this.errorLog = [];
    this.config = config;
  ...
  async processCodeBlock(codeBlock) {
    try {
      ...
      if (!resolved) {
        ...
        const error = {
          type: "LANGUAGE_NOT_SUPPORTED" /* LANGUAGE_NOT_SUPPORTED */,
      ...
      try {
        ...
        if (this.config.transformers.diff) {
          transformers.push(
            transformerNotationDiff({
              ...
              matchAlgorithm: this.config.diffOptions.matchAlgorithm
            })
          );
        }
      } catch (error) {
        const err = error;
        const processError = {
          type: "TRANSFORMER_ERROR" /* TRANSFORMER_ERROR */,
      ...
      try {
        ...
        if (err.message.includes("theme") || err.message.includes("Theme")) {
          const processError = {
            type: "THEME_NOT_FOUND" /* THEME_NOT_FOUND */,
          ...
          const processError = {
            type: "RENDERING_ERROR" /* RENDERING_ERROR */,
   ...
   * 批量处理代码块
   */
  async processCodeBlocks(codeBlocks) {
    const results = [];
   ...
   * 并行处理代码块
   */
  async processCodeBlocksParallel(codeBlocks, concurrency = 4) {
    const results = new Array(codeBlocks.length);
    const chunks = [];
...
// scripts/shiki/cache.ts
import { createHash } from "crypto";
import { readFile, writeFile, mkdir } from "fs/promises";
import { join, dirname } from "path";
var ShikiCache = class {
  // 缓存有效期（毫秒）
  constructor(cacheDir = ".shiki-cache", maxAge = 36e5) {
...
// scripts/shiki/markdown-scanner.ts
...
var MarkdownScanner = class {
   ...
   * 获取修改过的文件（基于时间戳）
   */
  async getModifiedFiles(lastBuildTime, pattern) {
    const files = await this.scanFiles(pattern);
    const modifiedFiles = [];
    for (const file of files) {
      try {
...
// scripts/shiki/config-validator.ts
...
var ConfigValidator = class {
   ...
   * 验证主题配置
   */
  validateThemes(config, errors, _warnings) {
    if (!config.themes) {
      errors.push({
        field: "themes",
   ...
   * 验证 transformers 配置
   */
  validateTransformers(config, errors, warnings) {
   ...
   * 验证 fileName 选项
   */
  validateFileNameOptions(config, errors, warnings) {
    ...
    if (typeof fileNameOptions.hideDot !== "boolean" && fileNameOptions.hideDot !== void 0) {
      errors.push({
   ...
   * 验证 diff 选项
   */
  validateDiffOptions(config, errors, warnings) {
   ...
   * 验证其他选项
   */
  validateOtherOptions(config, errors, _warnings) {
    if (typeof config.defaultColor !== "boolean" && config.defaultColor !== void 0) {
      errors.push({
        field: "defaultColor",
    ...
    if (typeof config.wrap !== "boolean" && config.wrap !== void 0) {
      errors.push({
        field: "wrap",
   ...
   * 查找相似的主题
   */
  findSimilarThemes(theme, maxResults = 5) {
    ...
    return results.sort((a, b) => b.score - a.score).slice(0, maxResults).map((r) => r.theme);
  }
  /**
   * 计算 Levenshtein 距离
   */
...
[${error.field}]`);
        ...
        if (error.suggestion) {
          console.error(`  \u{1F4A1} ${error.suggestion}`);
        }
      }
      console.error("\n" + "=".repeat(60) + "\n");
    }
...
[${warning.field}]`);
        ...
        if (warning.suggestion) {
          console.warn(`  \u{1F4A1} ${warning.suggestion}`);
        }
      }
      console.warn("\n" + "=".repeat(60) + "\n");
    }
...
var isMainModule2 = process.argv[1] && (import.meta.url.endsWith(process.argv[1]) || import.meta.url.includes(process.argv[1].replace(/\\/g, "/")));
if (isMainModule2) {
  main2();
}

// scripts/shiki/build.ts
var ShikiBuild = class {
  constructor(options = {}) {
    this.processor = null;
    this.cache = null;
    this.scanner = null;
    ...
    this.buildTimeFile = join2(this.options.cacheDir, "build-time.json");
  }
  /**
   * 读取上次构建时间
   */
  async getLastBuildTime() {
    try {
      ...
      const content = await readFile3(this.buildTimeFile, "utf-8");
  ...
  async saveBuildTime() {
    ...
    await writeFile2(this.buildTimeFile, JSON.stringify(record, null, 2), "utf-8");
  }
  /**
   * 检查文件是否需要处理
   */
  async shouldProcessFile(filePath, lastBuildTime) {
  ...
  async processBlock(block) {
    ...
    if (cached) {
      ...
      this.log(`\u2713 Cache hit: ${block.file}:${block.line} (${duration2}ms)`, true);
      return cached;
    }
   ...
   * 处理文件的所有代码块
   */
  async processFile(filePath, blocks) {
    const startTime = Date.now();
    const relativePath = relative(this.options.contentDir, filePath);
    this.log(`\u{1F4C4} Processing ${relativePath} (${blocks.length} blocks)`);
    const results = [];
   ...
   * 保存处理结果
   */
  async saveResults(filePath, results) {
    const relativePath = relative(this.options.contentDir, filePath);
    ...
    await writeFile2(outputPath, JSON.stringify(output, null, 2), "utf-8");
  }
  /**
   * 运行构建
   */
...
${progressBar} ${progress}% (${processedFiles}/${this.stats.totalFiles})`
      ...
      console.log(
        `Processed: ${this.stats.processedBlocks}, Cached: ${this.stats.cachedBlocks}, Failed: ${this.stats.failedBlocks}`
      );
    }
    console.log("\n" + "\u2500".repeat(60) + "\n");
   ...
   */
  printStats() {
    const duration = ((this.stats.endTime - this.stats.startTime) / 1e3).toFixed(
      2
    );
    console.log("\n" + "=".repeat(60));
    ...
    if (this.stats.totalBlocks > 0) {
      const avgTime = ((this.stats.endTime - this.stats.startTime) / this.stats.totalBlocks).toFixed(2);
...
var isMainModule3 = process.argv[1] && (import.meta.url.endsWith(process.argv[1]) || import.meta.url.includes(process.argv[1].replace(/\\/g, "/")));
if (isMainModule3) {
  main3();
}
export {
